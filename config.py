...
SCRAPERS = {
    ...
    "sayyarat": str(RAW_DATA_DIR / "sayyarat.csv"),
    "autocentral": str(RAW_DATA_DIR / "autocentral.csv"),
}
...
# Seuils de deals (importés par detect_deals.py -- source unique).
# SEUIL_DEAL_MIN = 0.25 (et non 0.15) : l'erreur relative médiane du modèle
# est ~12%, donc un seuil à 15% est À L'INTÉRIEUR du bruit d'estimation. Porté
# à ~2x l'erreur typique pour que l'écart devienne un vrai signal.
SEUIL_DEAL_MIN = 0.25    # Au moins 25% sous le prix théorique
SEUIL_DEAL_MAX = 0.55    # Au-delà de 55%, c'est presque toujours une erreur de prix

# Une estimation appuyée sur trop peu de comparables n'est pas fiable :
# en dessous de ce nombre d'annonces comparables, pas d'alerte Telegram.
...
# ============================================================================
# Source UNIQUE, importée par merging_files.py ET nettoyer_base.py. Avant,
# chaque module avait sa propre liste et elles divergeaient silencieusement.
MOTS_BRUIT = [
    ...
    "chaise", "pièce", "piece de rechange", "rechange", "accessoire", "siège",
...
# SOURCE UNIQUE de vérité pour les features du modèle. modele_prediction.py
# et test_modele.py DOIVENT importer ces listes -- ne pas les redéfinir en local.
#
# Jeu RÉDUIT (audit d'ablation) : 5 variables retirées sans perte de précision
# (MdAPE 10,50 % -> 10,58 %, dans le bruit) car mortes ou redondantes :
#   - Année            : r=1,00 avec Age_Vehicule (strictement la même info)
#   - Puissance_DIN    : r=0,87 avec Puissance_Fiscale, remplie à 61 % seulement
#   - Est_Presque_Neuve: couverte par Age_Vehicule (retrait = 0,00 pt)
#   - Nb_Options       : remplie à 3 % (quasi vide -> encode surtout la provenance)
#   - Boite_Vitesse    : boîte de vitesses (Manuelle/Automatique), harmonisée entre sources
# Features de production. Zone_Economique est volontairement EXCLUE du modèle
# de prix : sur les données actuelles elle est fortement confondue avec la source
# (automobile.tn est beaucoup plus Grand Tunis et plus haut de gamme que Tayara).
# Elle reste calculée dans la base pour les analyses géographiques, et son apport
# est mesuré séparément dans les diagnostics de biais.
FEATURES_NUMERIQUES = [
    "Kilométrage", "Log_Kilometrage", "Age_Vehicule", "Age_Carre",
    "Km_Par_An", "Puissance_Fiscale", "Cylindree", "Segment_Vehicule"
]

FEATURES_CATEGORIELLES = [
    "Marque", "Modèle", "Energie", "Boite_Vitesse"
]

# Drapeaux d'état dérivés du TEXTE des annonces (enrichir_base_avance.py les
# crée déjà). Ils ne portent un signal qu'une fois la DESCRIPTION scrapée
# (voir les scrapers : champ "Description" à remonter), suivie d'un re-scraping.
# À CE MOMENT-LÀ seulement, ajoute-les à FEATURES_NUMERIQUES ci-dessus puis
# relance `python core/modele_prediction.py` :
#   "Premiere_Main", "Non_Accidentee", "Full_Options", "Etat_Origine"
# (Les laisser hors du modèle tant qu'ils sont à 0 évite d'ajouter des
# variables mortes et de désynchroniser le .pkl des données.)
