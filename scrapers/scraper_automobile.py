...
COLONNES_FINALES = [
    ...
    "Nb_Options", "Options", "Description",
]
...
def extraire_specs_dynamiques(html_page, url_annonce):
    ...
    # Texte libre de la fiche. On ne prend qu'un bloc suffisamment long pour
    # éviter de confondre un label de navigation avec la description vendeur.
    infos_standard["Description"] = None
    for sel in [
        ".description-wrapper", ".car-description", ".announcement-description",
        "[class*='description']",
    ]:
        ...
        if el:
            ...
            if len(txt) >= 20:
                infos_standard["Description"] = txt
                break
