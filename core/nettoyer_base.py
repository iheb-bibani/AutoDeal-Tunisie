...
from config import (
    PROCESSED_FILES, MAX_DAYS_OLD, MOTS_BRUIT,
    MARQUES_ALIAS, MODELE_IMPLIQUE_MARQUE, MODELES_PAR_MARQUE_REFERENTIEL,
)


# ---------------------------------------------------------------------------
# Bruit : MOTS_BRUIT est désormais défini dans config (source unique),
# partagé avec merging_files.py.
# ---------------------------------------------------------------------------
...
def construire_devineur_modele(modeles_connus_par_marque):
    ...
    def deviner(marque, titre: str):
        ...
        # On a perdu les frontières de mots en retirant séparateurs : un `in`
        # naïf fait matcher un modèle court À L'INTÉRIEUR d'un autre mot
        # ("TT" dans "quaTTro", "Q5" dans "audiQ5..."). On limite donc ce repli
        # aux modèles d'au moins 4 caractères normalisés, là où une collision
        # de sous-chaîne est improbable ; les modèles courts restent gérés par
        # le niveau 1 (frontières de mots) et le niveau 2.
        titre_norm = normaliser_pour_comparaison(titre)
        for modele in candidats:
            modele_norm = normaliser_pour_comparaison(modele)
            if len(modele_norm) >= 4 and modele_norm in titre_norm:
                return modele
...
def process_data():
    ...
    # -- Harmonisation multi-source de la BOÎTE DE VITESSES. `Transmission`
    # désigne ici la transmission aux roues (traction/intégrale/propulsion) et
    # ne doit PAS être utilisée comme synonyme de boîte.
    boite = df["Boite_Vitesse"].astype("string").str.strip().str.lower()
    auto = boite.str.contains(r"automatique|automatic|\bauto\b|\bbva\b|dsg|cvt|edc|tiptronic|s-tronic", regex=True, na=False)
    manu = boite.str.contains(r"manuelle|manual|mécanique|mecanique|\bbvm\b", regex=True, na=False)
    df.loc[auto, "Boite_Vitesse"] = "Automatique"
    df.loc[manu, "Boite_Vitesse"] = "Manuelle"
    df.loc[~(auto | manu) & df["Boite_Vitesse"].notna(), "Boite_Vitesse"] = pd.NA
    ...
    # -- Véhicules non standards : épaves, sans papiers, pour pièces, moteur HS.
    # Leur prix est structurellement incomparable à celui d'une voiture roulante
    # et les laisser dans l'entraînement crée de fausses « affaires ». Pour le
    # terme accidenté, on protège explicitement « non/jamais accidenté ».
    texte_etat = (df["Titre"].fillna("").astype(str) + " " +
                  df.get("Description", pd.Series("", index=df.index)).fillna("").astype(str)).str.lower()
    motif_hs = r"\b[eé]pave\b|sans carte grise|pour pi[eè]ces|moteur\s+hs|non\s+roulant|[aà]\s+r[eé]parer|sinistr[eé]"
    mask_hs = texte_etat.str.contains(motif_hs, regex=True, na=False)
    mention_accident = texte_etat.str.contains(r"accident[eé]", regex=True, na=False)
    mention_saine = texte_etat.str.contains(r"non\s+accident|jamais\s+accident|aucun\s+accident|pas\s+accident", regex=True, na=False)
    mask_hs |= mention_accident & ~mention_saine
    if mask_hs.sum():
        print(f"🧹 {mask_hs.sum()} véhicules non standards écartés du modèle de marché (épave/accident/papiers/HS).")
    df = df[~mask_hs]
    ...
    # -- Dates : nos scrapers écrivent en ISO (AAAA-MM-JJ). Certaines sources
    # (AutoCentral/Sayyarat) ne publient pas de date de dépôt exploitable. Dans
    # ce cas on utilise la date de première détection : une annonce fraîchement
    # découverte ne doit pas disparaître du dataset ML uniquement parce que le
    # site ne fournit pas sa date de publication.
    df["Annonce-Deposee"] = pd.to_datetime(df["Annonce-Deposee"], errors="coerce")
    detectee = pd.to_datetime(df.get("Annonce-Detectee"), errors="coerce")
    df["Date_Reference_Annonce"] = df["Annonce-Deposee"].fillna(detectee)
    aujourd_hui = pd.Timestamp.now().normalize()
    df["Age_Annonce_Jours"] = (aujourd_hui - df["Date_Reference_Annonce"]).dt.days.clip(lower=0)
